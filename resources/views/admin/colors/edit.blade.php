        ...
        <div class="p-6 text-gray-900">
            <form action="{{ route('admin.colors.update', $color) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('name', $color->name) }}" required>
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-1 flex">
                        <input type="color" name="code" id="code" class="h-10 w-20 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" value="{{ old('code', $color->code) }}">
                        <input type="text" name="code_text" id="code_text" class="ml-2 block w-32 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('code', $color->code) }}" required>
                    </div>
                    @error('code')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="active" id="active" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <option value="1" {{ old('active', $color->active) == '1' ? 'selected' : '' }}>Ativo</option>
                        <option value="0" {{ old('active', $color->active) == '0' ? 'selected' : '' }}>Inativo</option>
                    </select>
                    @error('active')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
