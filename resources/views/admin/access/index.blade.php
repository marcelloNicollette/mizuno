...
@section('content-wrapper')
    @php
        $oldApprovalRequestId = old('approval_request_id');
        $approvalModalOpen = $errors->has('classification') && !empty($oldApprovalRequestId);
    @endphp

    <div x-data="{
        rejectionModalOpen: false,
        rejectionAction: '',
        approvalModalOpen: @js($approvalModalOpen),
        approvalAction: @js($oldApprovalRequestId ? route('admin.access.approve', $oldApprovalRequestId) : ''),
        approvalRequestId: @js($oldApprovalRequestId),
        approvalClassification: @js(old('classification', ''))
    }">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if (!$approved && !$exists && !$rejected)
                                        <div class="flex space-x-2">
                                            <button type="button"
                                                @click="approvalModalOpen = true; approvalAction = '{{ route('admin.access.approve', $request->id) }}'; approvalRequestId = '{{ $request->id }}'; approvalClassification = ''"
        ...
        <!-- Approval Modal -->
        <div x-show="approvalModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto"
            aria-labelledby="approval-modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div x-show="approvalModalOpen" x-transition:enter="ease-out duration-300"
                    x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                    x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                    aria-hidden="true"
                    @click="approvalModalOpen = false; approvalAction = ''; approvalRequestId = ''; approvalClassification = ''">
                </div>
                ...
                <div x-show="approvalModalOpen" x-transition:enter="ease-out duration-300"
                    ...
                    <form method="POST" :action="approvalAction">
                        @csrf
                        <input type="hidden" name="approval_request_id" :value="approvalRequestId">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div
                                    class="mt-3 mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-indigo-100 sm:mx-0 sm:h-10 sm:w-10">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    ...
                                    <div class="mt-2">
                                        <label for="approval_classification"
                                            class="block text-sm font-medium text-gray-700">
                                            Classificação
                                        </label>
                                        <select name="classification" id="approval_classification"
                                            x-model="approvalClassification"
                                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                            required>
                                            <option value="">Selecione a classificação</option>
                                            <option value="admin-user">Administrador Usuário</option>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="submit"
                                class="mt-3 mr-3 w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:ml-3 sm:w-auto sm:text-sm">
                                Aprovar
                            </button>
                            <button type="button"
                                ...
                                @click="approvalModalOpen = false; approvalAction = ''; approvalRequestId = ''; approvalClassification = ''">Cancelar</button>
                        </div>
                    </form>
                </div>
            </div>
