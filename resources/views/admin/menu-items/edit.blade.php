...
@section('page_title', 'Olympikus - Editar Item de Menu')

@section('content-wrapper')
    <div class="flex justify-between items-center mb-6">
        <h2 class="font-semibold text-2xl text-gray-800 leading-tight">
            {{ __('Editar Item de Menu') }}
        </h2>
                        ...
                        @foreach ($classifications as $classification)
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="allowed_classifications[]" value="{{ $classification }}"
                                    class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                                    {{ in_array($classification, $menuItem->allowed_classifications ?? []) ? 'checked' : '' }}>
