<x-layout-user title="Mizuno - Compartilhar">
    <style>
        .height-ultra {
    ...
    <main class="lg:flex flex-1 produtos-page">
        <!-- Menu lateral -->
        <x-sidebar activeItem="compartilhar" />

        <!-- Conteúdo principal -->
        <section class="flex-1 flex flex-col md:pr-0 md:pb-0 overflow-hidden">
            <!-- Filtros superiores -->
            <div class="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 pt-4 pb-3 pr-4 bg-[#F1F1F1]">
                <!-- Esquerda: Coleção e Histórico -->
                <div class="flex gap-2 w-full lg:w-auto">
                    <div class="select-container flex-1 lg:flex-initial">
                        ...
                        <div class="options min-w-[200px] p-5" id="options">
                            @foreach ($colecoes as $colecao)
                                <div class="option text-[18px]" data-slug="{{ $colecao->slug }}"
                                    ...
                                    <span class="option-content title" style="">

                                        @if ($colecao->description)
                                            {{ $colecao->name }} <span
                                                class="text-[14px] line-height-[16px] text-[#000] ml-[10px] description">{{ $colecao->description }}</span>
                                        @else
                                            {{ $colecao->name }}
                                        @endif
                                    </span>
                            ...
                            @endforeach
                            <div class="option ml-[20px]" data-value="todas">Todas</div>
                        </div>
                    </div>

                    <button id="openSharedHistoryModal"
                        class="flex items-center justify-center bg-[#e7e7e7] rounded-lg px-5 py-2 border-[#e7e7e7] border hover:border-black hover:border whitespace-nowrap">
                ...
                <div class="flex flex-col lg:flex-row lg:flex-wrap gap-2 lg:items-end lg:justify-end w-full lg:w-auto">
                    <div class="w-full lg:w-auto">
                        <div class="flex items-center border-b border-b-black px-2">
                            ...
                            <input type="text" placeholder="Buscar"
                                class="input-estilizado bg-transparent border-0 focus:outline-none focus:ring-0 p-1 w-full" />
                        </div>
                    </div>
            ...
            <template id="template-colecoes">
                <div class="colecao-card relative rounded overflow-hidden cursor-pointer border border-transparent hover:border-[#999]"
                    data-codigo="" data-name="" data-description="" data-slug="" data-segmentacao-id=""
                    onclick="openCompartilharModalWithCollection(this.dataset.name, this)">
            ...
            <x-shared-history-modal :sharedCollections="$sharedCollections" />
    ...
    @push('scripts')
        <script>
            ...
            optionElements.forEach(optElement => {
                optElement.addEventListener('click', function(e) {
                    ...
                    if (option && option.classList.contains('option')) {
                        ...
                        if (optionContent) {
                            // Extract text excluding children (description)
                            text = Array.from(optionContent.childNodes)
                                .filter(node => node.nodeType === Node.TEXT_NODE)
                                .map(node => node.textContent.trim())
                                .join(' ');

                            const descSpan = optionContent.querySelector('span');
                            if (descSpan) {
                        ...
                        const colecaoSelectedText = document.getElementById('colecaoSelectedTitle');
                        if (colecaoSelectedText) {
                            colecaoSelectedText.textContent = (description === '' || !description) ?
                                'Coleção: ' + text :
                                'Coleção: ' + text + ' ' + description;
                        }

                        // Update selected value for filtering
                        selectedColecaoValue = option.getAttribute('data-value');
            ...
            // Inicialização do Modal de Histórico Compartilhado
            const openSharedHistoryBtn = document.getElementById('openSharedHistoryModal');
            const sharedHistoryModal = document.getElementById('sharedHistoryModal');

            if (openSharedHistoryBtn && sharedHistoryModal) {
                openSharedHistoryBtn.addEventListener('click', function() {
                    sharedHistoryModal.classList.remove('hidden');
                });
            }
        </script>
    @endpush
