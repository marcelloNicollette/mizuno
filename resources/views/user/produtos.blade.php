<x-layout-user title="Mizuno - Produtos">
    ...
    <main class="lg:flex flex-1 produtos-page">
        <style>
            ...
            #pedidoSaveBtn.is-saved {
                color: rgba(0, 0, 0, 0.2);
                text-decoration-color: rgba(0, 0, 0, 0.2);
                cursor: default;
            }

            #pedidoSaveBtn.is-saved:disabled {
                color: rgba(0, 0, 0, 0.2);
                text-decoration-color: rgba(0, 0, 0, 0.2);
            }
                ...
                <div id="vitrineModalBody">
                    <div id="vitrineGridWrap">
                        <div id="vitrineTitleRow">
                            ...
                            <button id="vitrineRenameBtn" type="button">Renomear</button>
                        </div>
    ...
    @push('scripts')
        <script>
            ...
            const pedidoSavedSnapshotKey = "pedido_saved_snapshot_v1";
            ...
            const pedidoSaveBtnDefaultHtml = pedidoSaveBtn ? pedidoSaveBtn.innerHTML : '';
            const pedidoSaveBtnSavedHtml = `<svg xmlns="http://www.w3.org/2000/svg" width="28" height="29" viewBox="0 0 28 29" fill="none">
<path d="M4.77567 29H23.2243C26.4183 29 28 27.3617 28 24.1167V4.88321C28 1.63824 26.4183 0 23.2243 0H4.77567C1.59696 0 0 1.62249 0 4.88321V24.1167C0 27.3774 1.59696 29 4.77567 29ZM9.36883 23.0928C8.85172 23.0928 8.51712 22.6991 8.51712 22.0375V8.36445C8.51712 6.78923 9.308 5.95435 10.8289 5.95435H17.1863C18.7225 5.95435 19.5134 6.78923 19.5134 8.36445V22.0375C19.5134 22.6991 19.1787 23.0928 18.6464 23.0928C18.2358 23.0928 18.0076 22.8408 17.2928 22.1004L14.0837 18.8555C14.038 18.8082 13.9772 18.8082 13.9316 18.8555L10.7377 22.1004C10.0076 22.8408 9.77948 23.0928 9.36883 23.0928Z" fill="black"/>
</svg> Salvar`;
            ...
            function getPedidoSnapshot() {
                const title = getPedidoTitle();
                const normalizedTitle = title === 'Altere o título do pedido para salvar' ? '' : title;
                const keys = Array.from(pedidoItens.keys()).filter(Boolean).sort();
            ...
            function getPedidoSavedSnapshot() {
                try {
                    return localStorage.getItem(pedidoSavedSnapshotKey) || '';
                } catch (e) {
                    return '';
                }
            }

            function setPedidoSavedSnapshot(snapshot) {
                try {
                    localStorage.setItem(pedidoSavedSnapshotKey, snapshot);
                } catch (e) {}
            }

            function clearPedidoSavedSnapshot() {
                try {
                    localStorage.removeItem(pedidoSavedSnapshotKey);
                } catch (e) {}
            }

            function isPedidoSaved() {
                if (pedidoItens.size === 0) return true;
                const saved = getPedidoSavedSnapshot();
                if (!saved) return false;
                return saved === getPedidoSnapshot();
            }

            function marcarPedidoComoSalvo() {
                setPedidoSavedSnapshot(getPedidoSnapshot());
                syncPedidoSaveUi();
            }

            function syncPedidoSaveUi() {
                if (!pedidoSaveBtn) return;
                const saved = isPedidoSaved();
                pedidoSaveBtn.classList.toggle('is-saved', saved);
                pedidoSaveBtn.disabled = saved;
                pedidoSaveBtn.setAttribute('aria-disabled', saved ? 'true' : 'false');
                const desiredHtml = saved ? pedidoSaveBtnSavedHtml : pedidoSaveBtnDefaultHtml;
                if ((pedidoSaveBtn.innerHTML || '').trim() !== desiredHtml.trim()) {
                    pedidoSaveBtn.innerHTML = desiredHtml;
                }
            }
            ...
            if (pedidoOverlayBackBtn) {
                pedidoOverlayBackBtn.addEventListener('click', (e) => {
                    ...
                    if (pedidoItens.size > 0) {
                        if (!isPedidoSaved()) {
                            if (!confirm(
                                    'Tem certeza de que deseja sair do pedido, se sair todos os itens serão perdidos?'
                                ))
                                return;
                        } else {
                            if (!confirm(
                                    'Este pedido ja foi salvo. Ao sair, o pedido atual será zerado. Deseja continuar?'
                                ))
                                return;
                        }
                        limparPedido();
            ...
            const vitrineRenameBtn = document.getElementById('vitrineRenameBtn');
            ...
            if (vitrineRenameBtn && vitrineTitleEl) {
                vitrineRenameBtn.addEventListener('click', (event) => {
                    event.preventDefault();
                    vitrineTitleEl.focus();
                    const selection = window.getSelection();
                    if (!selection) return;
                    const range = document.createRange();
                    range.selectNodeContents(vitrineTitleEl);
                    selection.removeAllRanges();
                    selection.addRange(range);
                });
            }
...
<path opacity="0.5" d="M3.82541 3.28334H4.97521V1.74585C4.97521 1.33633 5.26446 1.07049 5.69835 1.07049H8.2872C8.72107 1.07049 9.01035 1.33633 9.01035 1.74585V3.28334H10.1601V1.674C10.1601 0.63224 9.48038 0 8.36674 0H5.6188C4.50516 0 3.82541 0.63224 3.82541 1.674V3.28334ZM0.542355 3.8581H13.4649C13.7614 3.8581 14 3.60664 14 3.31207C14 3.01751 13.7614 2.77323 13.4649 2.77323H0.542355C0.253099 2.77323 0 3.01751 0 3.31207C0 3.61382 0.253099 3.8581 0.542355 3.8581ZM3.68802 16H10.3192C11.3533 16 12.0475 15.3318 12.0982 14.3044L12.6044 3.72159H11.4401L10.9556 14.1823C10.9411 14.6133 10.6302 14.9151 10.2035 14.9151H3.78925C3.37707 14.9151 3.06612 14.6062 3.04442 14.1823L2.53099 3.72159H1.39566L1.90909 14.3116C1.95971 15.339 2.63946 16 3.68802 16ZM4.8595 13.6865C5.1343 13.6865 5.31508 13.5141 5.30785 13.2627L5.08368 5.5752C5.07645 5.32374 4.89566 5.1585 4.63533 5.1585C4.36054 5.1585 4.17975 5.33093 4.18699 5.58239L4.40393 13.2627C4.41116 13.5213 4.59194 13.6865 4.8595 13.6865ZM7.00003 13.6865C7.27482 13.6865 7.47006 13.5141 7.47006 13.2627V5.58239C7.47006 5.33093 7.27482 5.1585 7.00003 5.1585C6.72518 5.1585 6.53722 5.33093 6.53722 5.58239V13.2627C6.53722 13.5141 6.72518 13.6865 7.00003 13.6865ZM9.14772 13.6865C9.40806 13.6865 9.58887 13.5213 9.59609 13.2627L9.81299 5.58239C9.82027 5.33093 9.63947 5.1585 9.36468 5.1585C9.10434 5.1585 8.92353 5.32374 8.91631 5.58239L8.69941 13.2627C8.69213 13.5141 8.87293 13.6865 9.14772 13.6865Z" fill="black"/>
            ...
            async function vitrineCaptureToCanvasManual() {
                ...
                const titleRowStyle = titleRow ? getComputedStyle(titleRow) : null;
                const titleRowRect = titleRow ? titleRow.getBoundingClientRect() : null;
                ...
                const headerGap = vitrineParsePx(titleRowStyle?.gap, 18);
                const headerH = Math.max(logoH || 0, Math.ceil(titleRowRect?.height || fontSize * 1.6));
                const canvasH = Math.ceil(padTop + headerH + gridHeaderGap + gridH + padBottom);
                ...
                const textAscent = textMetrics.actualBoundingBoxAscent || fontSize * 0.8;
                const textDescent = textMetrics.actualBoundingBoxDescent || fontSize * 0.2;
                const titleVisualOffsetY = 7;
                const groupGap = headerGap;
                const groupW = (logoImg ? (logoW || logoImg.width) + groupGap : 0) + textW;
                const groupX = Math.max(0, Math.floor((canvasW - groupW) / 2));
                const headerCenterY = padTop + Math.floor(headerH / 2);
                const titleBaselineY = headerCenterY + Math.floor((textAscent - textDescent) / 2) + titleVisualOffsetY;
                let xCursor = groupX;
                if (logoImg) {
                    const drawW = logoW || logoImg.width;
                    const drawH = logoH || logoImg.height;
                    ctx.drawImage(logoImg, xCursor, headerCenterY - Math.floor(drawH / 2), drawW, drawH);
                    xCursor += drawW + groupGap;
                }
                ctx.textBaseline = 'alphabetic';
                ctx.fillText(titleText, xCursor, titleBaselineY);

                const startX = paddingX;
                const startY = padTop + headerH + gridHeaderGap;
